...
import Form from "./studentForm/AdminForm";
...
import AdminList from "./studentlist/AdminList";
import { fetchAction } from "../../store/action/fetchAction";
import SearchIcon from "@material-ui/icons/Search";
import CloseIcon from "@material-ui/icons/Close";
import AdminInfo from "./Info/AdminInfo";
...
const Adding = (props) => {
  ...
  const [adminSumbitted,  setAdminSumbitted] = useState(false);
  const [loading, setLoading] = useState(false);
  const adminSubmit = () => {
    setAdminSumbitted(!adminSumbitted);
  };

  const abort = new AbortController();
  useEffect(() => {
    setLoading(true)
    fetch("https://eng-asu-lms.herokuapp.com/admins/getAdmins", {
      signal: abort.signal,
      method: "GET",
      headers: {
      ...
      .then((data) => {
        props.fetchAction("Admin done", data);
              ...
              {props.datal ? (
                ...
                >
                  { props.datal.filter((user) => user.name.includes(searchVal.toLowerCase()) ||user.code.toLowerCase().includes(searchVal.toLowerCase())).length === props.datal.length
                    ?  props.datal.filter((user) => user.name.includes(searchVal.toLowerCase()) ||user.code.toLowerCase().includes(searchVal.toLowerCase())).length + "'s Admins"
...
{props.datal ?(
        ...
        >
            <Form close = {close} adminSubmit={adminSubmit} />
          </Box>
       {props.datal ? (
          <Box
            className={clsx(
